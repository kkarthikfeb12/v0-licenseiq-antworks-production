...
export const seedLicenses: License[] = rawLicenseData.map((item, index) => {
  ...
  const defaultPayload: ApprovalPayload = {
    ...
    auth_method: "Mac ID",
    auth_value: "",
    mac_id: `00:1A:2B:3C:4D:${String(index).padStart(2, "0")}`,
    motherboard_serial_no: `MB${String(100000 + index)}`,
    processor_id: `PROC-${String(index + 1).padStart(4, "0")}`,
    c_drive_serial_no: `CD${String(200000 + index)}`,
    no_of_pages: String((index + 1) * 1000),
    no_of_documents: String((index + 1) * 100),
