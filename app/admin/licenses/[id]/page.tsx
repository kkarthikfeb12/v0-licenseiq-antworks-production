...
function AdminLicenseDetailContent({ licenseId }: { licenseId: string }) {
  const router = useRouter()
  const { getLicenseById, getAuditByLicense, getUserById } = useDataStore()
  ...
  const amUser = getUserById(license.am_id)
            ...
            <CardContent>
              <div className="grid gap-4 sm:grid-cols-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm text-muted-foreground">AM Email</p>
                    <p className="font-medium text-sm">{amUser?.email || "-"}</p>
                  </div>
                </div>
              </div>
            </CardContent>
...
export default function AdminLicenseDetailPage({ params }: { params: Promise<{ id: string }> }) {
